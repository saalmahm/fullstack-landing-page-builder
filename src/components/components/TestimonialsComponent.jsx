...
export default function TestimonialsComponent({ data, onEdit, isPreview, theme }) {
  ...
  if (editMode && !isPreview) {
    return (
      <div className="p-6 bg-yellow-50 border-2 border-yellow-200 rounded-xl">
        <h3 className="text-lg font-semibold mb-4 text-yellow-900">Modifier les Témoignages</h3>
  ...
  return (
    ...
    >
      ...
      {!isPreview && (
        <div className="absolute inset-0 bg-yellow-500 bg-opacity-0 hover:bg-opacity-10 transition-all duration-200 cursor-pointer rounded-lg">
          <div className="absolute top-2 left-2 bg-yellow-500 text-white px-2 py-1 rounded text-xs opacity-0 hover:opacity-100 transition-opacity">
